...
# -------------------------- find coherency ------------------------
def neighbor_func(arr):
    return sum(arr)

def Semblance_coherence(signals, paras):
    '''semblance-based coherence (Marfurt et al. 1998)
        reference: https://github.com/seg/tutorials-2015/blob/master/1512_Semblance_coherence_and_discontinuity/writeup.md
    '''
    # setup parameters
    ...
    sums_num = nd.generic_filter(square_sums, neighbor_func, semblance_nsmps, mode='constant')
    sums_den = Nb*nd.generic_filter(sum_squares, neighbor_func, semblance_nsmps, mode='constant')
    # semblance
    semblance = sums_num/sums_den
         
    # modified semblance
    semblance_modified = semblance**semblance_order*v 
    
    return v, sums_num, sums_den, semblance, semblance_modified   

def WCC_coherence(signals, paras):
    '''Moving-window crosscorrelation.
       reference: https://github.com/mdenolle/NoisePy/blob/master/src/noise_module.py
    ''' 
    # common variables 
    ntr, npts = signals.shape
    twin = paras['twin']
    ...
    dt = paras['dt']
    twind = paras['correlation_window']
    tstep = paras['moving_step']
    ntwind = int(twind/dt)
    ntstep = int(tstep/dt)
    t = npts*dt
    nsegs = np.floor((npts-ntwind+1)/ntstep)
    ...
    # remove variables and free space up
